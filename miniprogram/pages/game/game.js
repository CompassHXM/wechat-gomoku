...
Page({
    ...
    async initOnlineGame() {
        ...
        try {
            // 获取房间信息
            const room = await api_1.api.getRoom(this.data.roomId);
            if (!room) {
                wx.hideLoading();
                wx.showToast({
                    title: '房间不存在',
            ...
            // 判断玩家身份
            const myPlayer = room.players.find((p) => p.userId === this.data.myUserId);
    ...
    async connectWebSocket() {
        try {
            await websocket_1.wsManager.connect(this.data.myUserId, this.data.roomId);
            // 监听消息
            websocket_1.wsManager.onMessage((data) => {
    ...
    drawPiece(ctx, row, col, player) {
        ...
        // 添加高光效果
        const gradient = ctx.createRadialGradient(x - radius * 0.3, y - radius * 0.3, 0, x - radius * 0.3, y - radius * 0.3, radius);
        if (player === 1) {
    ...
    async makeOnlineMove(row, col) {
        try {
            ...
            await api_1.api.makeMove(this.data.myUserId, this.data.roomId, row, col);
            wx.hideLoading();
